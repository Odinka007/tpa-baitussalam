    ...
    <div class="table-responsive">
        <form action="{{ route('datasantri.index') }}" method="GET" class="mb-3">
            <div class="input-group input-group-sm col-md-4 mx-left">
                <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari nama santri..." value="{{ request('search') }}">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-danger btn-sm"  data-bs-toggle="modal" data-bs-target="#hapusModal{{ $santri->id }}">
                            <i class="bi bi-trash"></i> Hapus
                        </button>

                        <!-- Modal Konfirmasi Hapus -->
                        <div class="modal fade" id="hapusModal{{ $santri->id }}" tabindex="-1" aria-labelledby="hapusModalLabel{{ $santri->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content border-0 shadow">
                                    <div class="modal-header bg-danger text-white">
                                        <h5 class="modal-title" id="hapusModalLabel{{ $santri->id }}">Konfirmasi Hapus</h5>
                                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        Apakah kamu yakin ingin menghapus data santri <strong>{{ $santri->nama_santri }}</strong>?
                                    </div>
                                    <div class="modal-footer">
                                        <form action="{{ route('datasantri.destroy', $santri->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
