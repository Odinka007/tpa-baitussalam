...
@section('content')
    <div class="container">
        <h1>Edit Mata Pelajaran PAUD</h1>

        <form action="{{ route('matapelajaran.paud.update', $mapel->id) }}" method="POST">
            @csrf
            @method('PUT')
        ...
        <a href="{{ route('matapelajaran.paud.index') }}" class="btn btn-secondary mt-3">Kembali ke Daftar Mata Pelajaran</a>
    </div>
@endsection
